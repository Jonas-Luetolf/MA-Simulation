...
class Vector:
    def __init__(self, components):
        if not isinstance(components, (tuple, list)):
            raise TypeError("Vector only takes list or tuple")
    ...
    def __add__(self, other):
        if not isinstance(other, Vector):
            raise TypeError("addition only supported between Vector and Vector")

        if not len(self.components) == len(other.components):
            raise TypeError("addition only supported between Vectors with same size")
...
def angle(x: Vector, y: Vector, deg: bool = True) -> float:
    ...
    if not len(x.components) == len(y.components):
        raise ValueError(
            "angle only supported between two Vectors with same number of components"
        )
